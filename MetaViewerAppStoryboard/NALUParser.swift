...
final class NALUParser {
    ...
    func process(payload: Data, header: UDPPacketHeader) {
        ...
        let nalType = H264NALType.from(byte: payload[0])
        ...
        case .idr, .nonIDR:
            ...
            do {
                let sb = try buildSampleBuffer(payload: payload,
    ...
    private func buildSampleBuffer(payload: Data,
        ...
        if let attachmentArray = attachments as? [[CFString: Any]],
           let dict = (attachmentArray as CFArray).unsafelyUnwrapped as? [[CFString: Any]] {
            // Use direct CoreFoundation access
        }
