...
final class VideoRenderer: UIView {
    ...
    /// The format description is derived from the pixel buffer itself — no need to pass one in.
    func render(pixelBuffer: CVPixelBuffer,
                presentationTime: CMTime) {
        // IMPORTANT: CMSampleBufferCreateForImageBuffer requires a format description
        // that matches the *decoded* pixel buffer (e.g. 420v YCbCr), NOT the compressed
        // H264 description produced by CMVideoFormatDescriptionCreateFromH264ParameterSets.
        // Create the correct description directly from the pixel buffer itself.
        var pixelFmtDesc: CMFormatDescription?
        let descStatus = CMVideoFormatDescriptionCreateForImageBuffer(
            allocator:              kCFAllocatorDefault,
            imageBuffer:            pixelBuffer,
            formatDescriptionOut:   &pixelFmtDesc
        )
        guard descStatus == noErr, let fmtDesc = pixelFmtDesc else {
            NSLog("[VideoRenderer] Failed to create format description from pixel buffer: %d", descStatus)
            return
        }
        ...
        guard status == noErr, let sb = sampleBuffer else {
            NSLog("[VideoRenderer] CMSampleBufferCreateForImageBuffer failed: %d", status)
            return
        }
