...
final class BLECentralManager: NSObject {
    ...
    private var streamObjectNameChar: CBCharacteristic?
    ...
    func sendObjectNameStreamer(name: String) {
        guard let peripheral = discoveredPeripheral,
              let char = streamObjectNameChar,
              peripheral.state == .connected else {
            NSLog("[BLECentral] Cannot notify — not connected to peripheral")
            return
        }
        ...
        // Use write without response for speed (fire-and-forget)
        peripheral.writeValue(data, for: char, type: .withoutResponse)
        NSLog("[BLECentral] Send Object to streamer: %@", name)
    }
...
extension BLECentralManager: CBPeripheralDelegate {
    ...
    func peripheral(_ peripheral: CBPeripheral, didDiscoverServices error: Error?) {
        ...
        for service in services where service.uuid == BLEConstants.serviceUUID {
            peripheral.discoverCharacteristics(
                [BLEConstants.wifiCredentialsCharUUID, BLEConstants.streamReadyCharUUID, BLEConstants.streamObjectNameCharUUID],
                for: service
            )
        }
    }
    ...
    func peripheral(_ peripheral: CBPeripheral,
        ...
        for char in chars {
            ...
            if char.uuid == BLEConstants.streamObjectNameCharUUID {
                streamObjectNameChar = char
            }
