...
final class ViewerViewController: UIViewController {
    ...
    private let coreML = CoreMLManager()
    ...
    func process(image: CGImage) {
        ...
        let orientation = CGImagePropertyOrientation(deviceOrientation: deviceOrientation)
        coreML.classify(cgImage: image, orientation: orientation) { label in
            DispatchQueue.main.async {
                self.statusLabel.text = label
                if self.lastObjectName != label {
                    DispatchQueue.global(qos: .background).async {
                        // Background work
                        self.bleManager.sendObjectNameStreamer(name: label)
                    }
                }
                self.lastObjectName = label
            }
        }
    }
...
private extension CGImagePropertyOrientation {
    init(deviceOrientation: UIDeviceOrientation) {
        ...
        case .portraitUpsideDown:
            self = .leftMirrored // Front camera style; adjust if your image source differs
        case .landscapeLeft:
            self = .upMirrored
        case .landscapeRight:
            self = .down
...
extension ViewerViewController: NALUParserDelegate {
    ...
    func detectColor(_ image: CIImage) {
        ...
        print("Average color → R:\(r) G:\(g) B:\(b)")
        self.connectButton.backgroundColor = UIColor(red: CGFloat(r)/255, green: CGFloat(g)/255, blue: CGFloat(b)/255, alpha: 1)
    }
}
...
extension ViewerViewController: H264DecoderDelegate {
    ...
    func decoder(_ decoder: H264Decoder,
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            if frameCounter % 170 == 0 {
                let image = self.convertCIImageToCGImage(ciImage: CIImage(cvPixelBuffer: pixelBuffer))
